...
//static const char* OLLAMA_URL = "https://...workers.dev/api/generate";
...
void ai_begin()
{
  ...
  if (gToken.length() == 0) {
    ...
    if (!warned) {
      ...
      Serial.println("\n AI TOKEN SETUP ");
      Serial.println("No token in NVS.");
      Serial.println("Run: SET_TOKEN <token>");
    }
  }
}
...
String ai_sendMessage(const String& userMessage)
{
  ...
  String prompt =
    "Reply in a few short sentences. No lists.\n"
    "User: " + userMessage + "\nAssistant:";
  ...
  if (out.length() > 250) out = out.substring(0, 250) + "...";
  return out;
}
